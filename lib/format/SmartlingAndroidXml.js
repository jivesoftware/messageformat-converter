...
var _AndroidXml = require('./AndroidXml');

var _AndroidXml2 = _interopRequireDefault(_AndroidXml);
...
var SmartlingAndroidXmlFormatter = Object.assign({}, _AndroidXml2.default, {
    nameIn: function nameIn(name) {
        return name.replace(/([^_])_([^_])/g, '$1.$2').replace(/__/g, '_');
    },
    nameOut: function nameOut(name) {
    ...
    stringIn: function stringIn(str) {
        ...
        var key = SmartlingAndroidXmlFormatter.nameIn(body.$.name);
        var tokenNames = body.$['messageformat:keys'] ? body.$['messageformat:keys'].split(',') : [];
        var mfString;
        var output = null;
        ...
        if (type === 'string') {
            mfString = body._;
            tokenNames.forEach(function (tokenName, index) {
                mfString = mfString.replace('%' + (index + 1) + '$s', '{' + tokenName + '}');
            });
            output = _MessageFormat2.default.stringIn([key, mfString]);
            // Case 2: <plurals>. Create a conversionString with one PluralBit.
                ...
                for (var i = 0, item; i < body.item.length; i++) {
                    ...
                    if (!(SmartlingAndroidXmlFormatter.ALLOWED_PLURAL_KEYS.indexOf(pluralAmount) >= 0)) {
                        throw new Error('Unknown plural key: ' + pluralKey);
                    }
                    var itemString = _MessageFormat2.default.stringIn([pluralAmount, item._.replace(/%d/g, '#')]);
    ...
    replaceNamedTokens: function replaceNamedTokens(conversionString, str, isPlural) {
        var tokenIndex = 1;
        var tokenNames = [];
        for (var i = 0, bit; i < conversionString.bits.length; i++) {
            ...
            if (bit.type === 'variable') {
                tokenNames.push(bit.varName);
                if (isPlural) {
                    str = str.replace('{' + bit.varName + '}', '%d');
                } else {
                    str = str.replace('{' + bit.varName + '}', '%' + tokenIndex++ + '$s');
                }
            }
        }
        return [str, tokenNames];
    },
    stringOut: function stringOut(conversionString) {
        ...
        if (typeof pluralBit !== "undefined" && pluralBit !== null) {
            ...
            ele.att('name', SmartlingAndroidXmlFormatter.nameOut(conversionString.key));
            ...
            for (var j = 0, pluralString; j < pluralBit.pluralStrings.length; j++) {
                ...
                if (SmartlingAndroidXmlFormatter.ALLOWED_PLURAL_KEYS.indexOf(ref) < 0) {
                ...
                var pluralCaseConversionString = new mfconv.ConversionString(pluralString.key, newBits);

                var _MessageFormatFormatt = _MessageFormat2.default.stringOut(pluralCaseConversionString);
                ...
                str = str.replace('#', '%d');
                var item = ele.ele('item', { quantity: key }, str);
            }
        } else {
            ...
            var _SmartlingAndroidXmlF = SmartlingAndroidXmlFormatter.replaceNamedTokens(conversionString, str);

            var _SmartlingAndroidXmlF2 = _slicedToArray(_SmartlingAndroidXmlF, 2);

            var str = _SmartlingAndroidXmlF2[0];
            var tokenNames = _SmartlingAndroidXmlF2[1];

            ele.att('name', SmartlingAndroidXmlFormatter.nameOut(key));
            if (tokenNames.length) {
                ele.att('messageformat:keys', tokenNames);
            }
            ele.txt(str);
        }
        return ele.toString();
        ...
        for (var i = 0, type; i < iterable.length; i++) {
            ...
            if (resources[type] != null) {
                ...
                for (var j = 0, str; j < resources[type].length; j++) {
                    ...
                    var elem = {};
                    elem[type] = str;
                    conversionStrings.push(SmartlingAndroidXmlFormatter.stringIn(elem));
                }
            }
        }
    ...
    fileOut: function fileOut(conversionFile) {
        ...
        for (var i = 0, conversionString; i < conversionFile.conversionStrings.length; i++) {
            ...
            var element = SmartlingAndroidXmlFormatter.stringOut(conversionString);
...
module.exports = SmartlingAndroidXmlFormatter;