...
import AndroidXml from './AndroidXml';

var SmartlingAndroidXmlFormatter = Object.assign({}, AndroidXml, {
    ...
    stringIn(str) {
        ...
        if (type === 'string') {
            ...
            output = MessageFormatFormatter.stringIn([key, mfString]);
        // Case 2: <plurals>. Create a conversionString with one PluralBit.
            ...
            for (var i = 0, item; i < body.item.length; i++) {
                ...
                var itemString = MessageFormatFormatter.stringIn([pluralAmount, item._.replace(/%d/g, `#`)]);
    ...
    replaceNamedTokens(conversionString, str, isPlural) {
        var tokenIndex = 1;
        var tokenNames = [];
        for (let i = 0, bit; i < conversionString.bits.length; i++) {
    ...
    stringOut(conversionString) {
        ...
        if ((typeof pluralBit !== "undefined" && pluralBit !== null)) {
            ...
            for (var j = 0, pluralString; j < pluralBit.pluralStrings.length; j++) {
                ...
                let newBits = conversionString.bits.slice(0);
                newBits.splice.apply(newBits, [pluralIdx, 1].concat(pluralString.bits));
                let pluralCaseConversionString = new mfconv.ConversionString(pluralString.key, newBits);
                var [key, str] = MessageFormatFormatter.stringOut(pluralCaseConversionString);
                str = str.replace('#', '%d');
            ...
            var [str, tokenNames] = SmartlingAndroidXmlFormatter.replaceNamedTokens(conversionString, str);
